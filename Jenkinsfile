node () {    
      ...
      stage('Build image') {         
           ...
           DOCKERHUB_CREDS = credentials('dockerhub')
            ...
            sh "docker build -t yenne1993/argocd-demo:test2 ."
            sh "docker login --username yenne1993 --password weLcome@#123 && docker push yenne1993/argocd-demo:test2"    
       }     
     ...
     stage('Deploy argocd') {
         ...
         dir("argocdmanifest") {
            ...
            cd ./charts/argocd-chart && yq eval '.image.tag |= "testkri"' -i values.yaml
